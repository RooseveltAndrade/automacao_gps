...
class GerenciadorServidores:
    ...
    def coletar_hardware_redfish(self, host):
        """
        Coleta hardware via Redfish.
        Retorna dict vazio se não suportado ou Redfish incompleto
        """
        try:
            url = f"https://{host}/redfish/v1/Systems"
            ...
            # tenta acessar o primeiro System real
            system_uri = members[0].get("@odata.id")
            if not system_uri:
                return {}

            system_url = f"https://{host}{system_uri}"
            r2 = requests.get(system_url, verify=False, timeout=5)
            ...
            # valida se tem dados mínimos de hardware
            if not system_data.get("ProcessorSummary") and not system_data.get("MemorySummary"):
                return {}
    ...
    def coletar_hardware(self, host, tipo=None):
        try:
            dados = self.coletar_hardware_redfish(host)
            if dados and dados.get("raw"):
                # valida se existe pelo menos 1 system válido
                return dados
            raise Exception
        except Exception:
            dados = self.coletar_hardware_snmp(host, tipo=tipo)
            dados['origem'] = 'snmp'
            return dados

    def detectar_tipo_snmp(self, host, community='public'):
        raw = self.consulta_snmp(
            host,
            '1.3.6.1.2.1.1.2.0',
            community
        )

        print("DEBUG sysObjectID RAW =>", repr(raw))
        ...
        raw = raw.replace('OID:', '').strip()
        raw = raw.replace('SNMPv2-SMI::', '').strip()

        print("DEBUG sysObjectID NORMALIZED =>", raw)

        if raw.startswith('enterprises.'):
            enterprise_id = raw.split('enterprises.')[-1].split('.')[0]
        elif raw.startswith('1.3.6.1.4.1.'):
            enterprise_id = raw.split('1.3.6.1.4.1.')[-1].split('.')[0]
        else:
            enterprise_id = ''

        print("DEBUG enterprise_id =>", enterprise_id)

        if enterprise_id == '674':
            return 'idrac'

        if enterprise_id == '232':
            return 'ilo'
    ...
    def coletar_hardware_snmp(self, host, tipo=None, community='public'):
        sysdescr = self.consulta_snmp(host, '1.3.6.1.2.1.1.1.0', community)
        sysname = self.consulta_snmp(host, '1.3.6.1.2.1.1.5.0', community)
        uptime = self.consulta_snmp(host, '1.3.6.1.2.1.1.3.0', community)

        # ------------------------------
        # DETECÇÃO DE TIPO
        # ------------------------------
        if not tipo:
            tipo = self.detectar_tipo_snmp(host, community)

        tipo_detectado = tipo

        descr_lower = (sysdescr or "").lower()
        name_lower = (sysname or "").lower()

        # Fallback por sysDescr / sysName
        if tipo_detectado == 'desconhecido':
            if 'integrated lights-out' in descr_lower or name_lower.startswith('ilo'):
                tipo_detectado = 'ilo'
            elif 'idrac' in descr_lower or name_lower.startswith('idrac'):
                tipo_detectado = 'idrac'

        print("TIPO DETECTADO FINAL:", tipo_detectado)
        ...
        dados_finais = {
            ...
            "cpu": {"count": None, "health": "N/A", "model": "N/A"},
            "memoria_gib": None,
            "temperaturas": [],
            "ventoinhas": []
        }
        ...
        if tipo_detectado == 'ilo':
            ...
            if temp:
                dados_finais['temperaturas'].append({
                    "sensor": "System",
                    "valor": int(temp)
                })

        # ------------------------------
        # Dell iDRAC (opcional futuro)
        # ------------------------------
        elif tipo_detectado == 'idrac':
            pass  # mantém compatibilidade, sem quebrar nada
